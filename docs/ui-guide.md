...
STA（Simple TUI Agent）使用原版 12 行橙色终端半块字符坐猫：尖耳、弯弯笑眼、并拢前爪和卷尾。图案直接由字符组成，不依赖终端的图片协议；下图由当前字符画生成，使用透明底矢量图；PNG 版本同步保留。

![STA 坐猫](images/sta-cat.svg)
...
3. 用 ↑↓ 选择任务并按 Enter，或输入 `/resume <完整检查点ID>`。STA 核对该任务记录的文件，将变化说明和历史交给当前模型，再判断剩余工作。
