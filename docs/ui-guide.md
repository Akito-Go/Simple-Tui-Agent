...
界面示例由当前 Textual 组件的渲染数据按字符格生成 PNG，使用常规字重与等宽排版；猫咪的半块字符按整像素绘制，避免块面间出现细缝。欢迎页与对话示例使用 120×29 的横向布局，紧凑示例为 80×24，确认示例为 120×32。示例使用虚构工作区与内容，没有调用真实模型。布局测试验证输入框和确认选项可见、工具详情键盘展开及失败状态；终端字体、emoji 和颜色可能随终端而异。
