...
1. **有历史会话时**：界面会列出最近会话（时间、模型、轮次、最后一条消息预览）
   - 输入 `1`–`9`：恢复对应会话
   - 输入 `N`：新建空会话
   - **直接输入消息**：新建会话并立即发送
   - 此阶段也可使用 `/help`、`/model`、`/status`、`/exit`
2. **无历史会话时**：直接进入对话，输入任务即可，例如：
   ```
   帮我看看项目结构
   ```
...
```
┌──────────────────────────────────────────────────────────────┐
│  TUI Agent · gpt-4o-mini · turn 1/50                   🟢   │
├──────────────────────────────────────────────────────────────┤
│  👤 帮我创建一个 Python 俄罗斯方块游戏                        │
│  🤖 我先看看项目结构...                                       │
│  ⚡ list_dir(path=.)   ✓ src/ tests/ ...                      │
│  ⚡ write_file(...)    ⚠ 需确认  [Y] 确认  [N] 拒绝           │
├──────────────────────────────────────────────────────────────┤
│  > 输入消息或命令 (/help /clear /stop /model /status /exit)  │
└──────────────────────────────────────────────────────────────┘
```
